...
export const MountainMap: React.FC<MountainMapProps> = ({
  ...
  const initialize3DMap = useCallback(() => {
    ...
    maplibreMap.current?.on('load', () => {
      ...
      allMountains.forEach((m) => {
        ...
        // Add click handler to the popup button
        marker.on('popupopen', () => {
          const button = document.querySelector('.mountain-popup button');
          if (button) {
  ...
  }, [mountain, allMountains, onMountainSelect, onClose]);

  // Initialize 2D map
  const initialize2DMap = useCallback(() => {
  ...
  }, [isOpen, is3DMode, initialize3DMap, initialize2DMap]);
